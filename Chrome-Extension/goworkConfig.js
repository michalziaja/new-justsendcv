// pracujConfig.js
window.GOWORK_CONFIG = {
    site: "gowork.pl",
    selectors: {
      applyButton: '[data-testid="first-application-button"]',
      position: 'h1[class^="css-"]',
    ...
    getJobInfo: (url) => {
      ...
      if (Object.keys(jobInfo.description).length === 0) {
        console.warn("Nie znaleziono sekcji opisu oferty na gowork.pl!");
      } else {
        console.log("Pobrano dane oferty z gowork.pl:", jobInfo);
      }
    ...
    getApplyButtonStyle: (applyButton) => {
      if (!applyButton) {
        console.info("Przycisk 'Aplikuj' nie znaleziony na gowork.pl z selektorem [data-test=\"button-submit\"] – używanie domyślnych stylów.");
        return {
          width: "100%",
          height: "auto",
          padding: "10px 20px",
